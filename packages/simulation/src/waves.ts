...
export function advanceWave(state: GameState, rng: SeededRandom, events: SimulationEvent[]): void {
  if (isDefeated(state)) { state.phase = "gameOver"; events.push({ type: "matchDefeated", tick: state.tick, wave: state.wave.currentWave }); return; }
  ...
  if (state.wave.currentWave >= Math.min(MAX_WAVES, state.wave.totalWaves)) { state.wave.matchComplete = true; state.phase = "gameOver"; events.push({ type: "matchCompleted", tick: state.tick, wave: state.wave.currentWave }); return; }
