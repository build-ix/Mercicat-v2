...
function snapshot(state: GameState, rng: SeededRandom): NetworkSnapshot { return { tick: state.tick, state: structuredClone(state), stateHash: hashGameState(state), rngState: rng.serialize() }; }
...
describe("Week 3 adverse network resilience", () => {
  it("recovers from latency, loss, reordering, and temporary loss with bounded queues", () => {
    ...
    for (let tick = 0; tick < 180; tick += 1) {
      ...
      if (tick % 23 !== 0) delayed.push(snapshot(server, rng));
    ...
    client.reconcile(snapshot(server, rng), Number.POSITIVE_INFINITY);
  ...
  it("never lets a client snapshot overwrite server gameplay authority", () => {
    ...
    const fake = snapshot(server, rng);
    ...
    const authoritative = snapshot(server, rng);
